...
however, you have to rely (almost) solely on the tiles you draw yourself
to advance your hand.
9
...
with , 
, and . Only one out of the three resulting protoruns is
side wait.
Terminals (1 and 9) are the least versatile. They can form a pro-
torun with only two kinds of tiles. For example,  can form a protorun
only with  and 
...
can become a side-wait one only if we draw . Likewise, a closedwait
protorun  can become a side-wait one only if we draw .
However, a closed-wait protorun 
 can become a side-wait one if
we draw  or . Clearly, 
...
The hand is complete only with  (1 kind–3 tiles). However, if we
...
a candidate for a set for two reasons. First, if we draw another ,
...
Drawing  or  will advance this hand from 2-away to 1-away.

Moreover, whereas the hand with one pair was able to accept two
tiles of , the hand with two pairs can accept four tiles (two of 
and two of ). The probability of drawing any one of four tiles
...
‌ 
Draw
If we keep the second  and discard the  or the , the hand has
...
additional tiles of . However, doing so comes with a cost. Keeping
three pairs by discarding the  means the hand can no longer
...
because ﬀ can become a side-wait protorun relatively easily. On
the other hand, the  shape and the 	 shape are both
weak; the ﬁrst can become a side-wait protorun only if we draw ,
...
diﬀerent terminology here might be confusing at ﬁrst, but I hope you will get
used to it soon.
6
...
 will make this hand 1-away for chiitoitsu (Seven Pairs).
8
...
10
Note that - wait means the winning tiles are  and , not  through .

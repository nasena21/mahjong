...
ways worthwhile to pursue this yaku at the cost of tile eﬃciency. We
...
From a pure tile eﬃciency perspective, the best discard choice is ﬂ.
...
tic possibility of getting ittsu. Let’s apply the ﬁve-block method to
ﬁgure out an alternative discard.
...
(bamboos). The choice now boils down to discarding  or . Recall
also that the value of pairs is maximized when there are two pairs
in a hand. We should thus discard .

## Six-tile block with intervals
Consider diﬀerent six-tile conﬁgurations where we have a chunk
...
aiming for ittsu with these blocks is not very realistic. Take the ﬁrst
six-tile block in manzu (cracks), for example. Even when we draw ,
we would want to discard  to have a double closed shape ȷ
and a run ﬀ rather than trying too hard to pursue ittsu. With
...
pursuing it requires we ﬁll in three closed-wait protoruns in souzu:
, , and 	.

It would be more practical to discard  and then 	; we would
consider the tile blocks in souzu as a collection of two side-wait protoruns:
and + two redundant terminal tiles:  	, rather
than considering it as a collection of three closed-wait protoruns.

## Run + side-wait protorun
...
suit. The following blocks are examples of such run + side-wait protorun
combinations.
‌+ﬀ
...
When we have a combination like these, a draw of  or  (left example),
 or  (middle example),  or  (right example) generates
a realistic probability of getting ittsu. Below are the resulting tile blocks in each instance. You can pursue ittsu with any of these
blocks.
...
one, the chance of getting ittsu is much diminished. The following
blocks are examples of such run + closed- or edge-wait protorun
...
If the run becomes a bulging ﬂoat block, you may want to give up on
...
Now that we drew a tile that creates a bulging ﬂoat block in manzu
...
from ittsu. Pursuing ittsu with a hand like this is not very practical.

## Ittsu vs. side wait
As we saw with sanshoku hands, we often face a choice between
pursuing yaku and keeping a side-wait protorun. Consider the following
1-away hand.
...
If we discard  we drew, the hand is a perfect 1-away hand; the ﬁnal
wait can always be side wait. On the other hand, if we discard ,
we have a conﬁrmed ittsu hand. Which option should we choose?

If we compare tile acceptance counts for the two scenarios, the
option of conﬁrming side wait is slightly better (6 kinds–18 tiles vs.
...
tiles are smaller, the second option would be more eﬃcient if we take
